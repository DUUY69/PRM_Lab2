...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _copyFcmToken() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Đã copy FCM token — dán vào Firebase Console test'),
      ),
    );
  }
  ...
  Future<void> _previewPdf() async {
    try {
      ...
      await AnalyticsService.logExportPdf(publicationProvider.currentTopic);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          _buildUserHeader(auth.currentUser),
          ...
          _buildFcmCard(),
          const SizedBox(height: 24),
          _buildSectionTitle('Notification Center'),
          const SizedBox(height: 10),
          ...
          const SizedBox(height: 24),
          _buildSectionTitle('Crashlytics'),
          const SizedBox(height: 10),
          _buildCrashlyticsCard(),
          const SizedBox(height: 24),
        ],
      ),
    );
  ...
  Widget _exportSubtitle() {
    if (_isUploading) return const Text('Uploading...');
    ...
    return const Text('Generate PDF and upload to Firebase');
  }

  Widget _buildAccountCard(BuildContext context, AuthViewModel auth) {
    return Card(
      child: Column(
        children: [
          ListTile(
          ...
          ListTile(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const Scaffold(body: AboutScreen()),
                ),
              );
            },
          ),
          ...
          child: Column(
            children: [
              ListTile(
                ...
                onTap: () => setState(() => _fcmExpanded = !_fcmExpanded),
              ),
      ...
      child: Column(
        children: [
          ListTile(
            ...
            trailing: Icon(
              _remoteConfigExpanded ? Icons.expand_less : Icons.expand_more,
            ),
            onTap: () {
              setState(() => _remoteConfigExpanded = !_remoteConfigExpanded);
            },
          ),
      ...
      child: Column(
        children: [
          ListTile(
            ...
            trailing: Icon(
              _crashlyticsExpanded ? Icons.expand_less : Icons.expand_more,
            ),
            onTap: () {
              setState(() => _crashlyticsExpanded = !_crashlyticsExpanded);
            },
          ),
